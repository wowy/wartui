...
///
/// Divergence 4, and worse than the table says: a re-used epoch is not only
/// ignored but still acknowledged, so the host believes an assignment landed
/// that the node discarded. Writing the counter
