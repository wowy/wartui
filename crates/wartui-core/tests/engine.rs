...
fn an_assignment_waits_for_the_heartbeat_that_opens_the_window() {
    ...
    let (id, _, alone) = sent_admin(&engine.handle(heartbeat(NODE, 1), clock.at(1)));
    assert_eq!(alone.epoch, 1, "the first epoch of a fresh database");
    engine.handle(send_result(id, SendStatus::AckOk, 900), clock.at(1));

    // A second node joins, so the pool is re-cut and the first node owes a
    // narrower share. Its radio is away for all but the 100 ms after its own
    // heartbeat, so nothing reaches it in somebody else's window.
    let joined = engine.handle(heartbeat(OTHER, 1), clock.at(2));
    let (_, dst, _) = sent_admin(&joined);
    assert_eq!(dst, OTHER, "only the node whose window is open is written to");
    assert!(engine.nodes().next().expect("the first node").dirty, "the other is owed one");
...
fn a_node_heard_only_through_its_observations_gets_nothing_yet() {
    ...
    let ticked = engine.handle(Event::Tick, clock.at(2));
    assert!(ticked.urgent.is_empty());
...
fn the_bluetooth_scan_waits_for_a_node_that_has_not_arrived_yet_however_long_that_takes() {
    ...
    // And when it does turn up, the flag is on the first frame the planner sends
    // it: nothing else had to be asked for.
...
fn an_assignment_is_believed_only_once_the_node_radio_acknowledges_it() {
    ...
    assert_eq!(node.confirmed.expect("confirmed").channels, ChannelPool::Us.channels());
...
fn a_lost_answer_expiring_late_does_not_unpick_an_assignment_that_has_since_landed() {
    ...
    assert_eq!(node.confirmed.expect("still confirmed").channels, ChannelPool::Us.channels());
...
fn a_reboot_re_issues_the_assignment_under_a_fresh_epoch() {
    ...
    assert_eq!(reissued.channels, first.channels, "and the share it was already holding");
...
fn an_acknowledgement_for_an_assignment_the_plan_has_already_replaced_is_not_believed() {
    ...
    let mut engine = engine(EngineConfig::default(), &clock);
    engine.handle(heartbeat(NODE, 1), clock.at(1));
    let (id, _, alone) = sent_admin(&engine.handle(heartbeat(NODE, 2), clock.at(6)));

    // A node joined while the first frame was in flight, so the pool was re-cut
    // and the ack that follows is for a share nobody holds any more.
    engine.handle(heartbeat(OTHER, 1), clock.at(6));
...
// ---------------------------------------------------------------------------
// Partitioning: the engine holding the whole fleet on a cut of the pool, which
// is what "replaces the core" means rather than "watches it".
// ---------------------------------------------------------------------------
...
fn a_fleet_is_partitioned_without_being_asked_and_nothing_can_stop_it() {
    ...
    // A fourth node joining is answered the same way, however long the capture
    // has been running.
    engine.handle(heartbeat(peer(3), 1), clock.at(3));
    let joined = engine.nodes().find(|node| node.mac == peer(3)).expect("the new node");
    assert!(joined.desired.is_some(), "and it is dealt a share on arrival");
    assert_eq!(covered(&wanted(&engine)), pool_indices(ChannelPool::Us));
}
...
fn nor_on_a_fleet_with_more_nodes_than_channels_to_give_them() {
    ...
    // empty set rather than a share worth keeping, so the rule that a node the plan
    // has nothing for keeps what it holds had to grow the case where it holds nothing.
    let clock = Clock::new();
    ...
    let check = |batch: &ActionBatch, seen_empty: &mut u32| {
        for admin in admins(batch) {
            assert_eq!(admin.channels.is_empty(), admin.scan_ble(), "one fact, said twice");
            if admin.channels.is_empty() {
                *seen_empty += 1;
            }
        }
    ...
    for (from, target) in [(2u64, Some(peer(12))), (20, Some(peer(0))), (40, None)] {
        engine.handle(Event::Command(Command::AssignBle { mac: target }), clock.at(from));
        for beat in 0..6u32 {
    ...
    assert!(seen_empty >= 2, "the scan really did move around: {seen_empty}");
}
...
/// Drive a fleet of `nodes` ESP32-C6s claiming `ble` until every assignment has
/// landed, answering each frame with an ack.
///
/// Twelve of these on `us` is the case worth having a helper for: eleven reachable
/// channels means one slot is always surplus, and `crates/wartui/README.md`
/// § "Channel pools" documents that as a fleet an operator may really run.
fn settle_c6_fleet(engine: &mut FleetEngine, clock: &Clock, nodes: u8, from: u64) {
    for beat in 0..6u32 {
        let at = clock.at(from + u64::from(beat));
...
fn the_bluetooth_scan_can_be_taken_off_a_node_the_planner_has_no_share_for() {
    ...
    let scanner = engine.nodes().find(|node| node.mac == peer(11)).expect("here");
    let held = scanner.confirmed.expect("it took the scan");
    assert!(held.ble && held.channels.is_empty(), "it really is the surplus slot: {held:?}");

    engine.handle(Event::Command(Command::AssignBle { mac: None }), clock.at(20));
    settle_c6_fleet(&mut engine, &clock, 12, 20);

    assert_eq!(engine.snapshot(clock.at(40), StoreStats::default()).ble_node, None);
    for node in engine.nodes() {
        let held = node.desired.or(node.confirmed).expect("in the plan");
        assert!(!held.ble, "{:02X?} is still scanning Bluetooth", node.mac);
        assert!(!held.channels.is_empty(), "{:02X?} was left with nothing to scan", node.mac);
    }
    // The surplus rule at its limit: it duplicates the whole fleet's work rather
    // than going blind, which is what a lone node holds too.
    let back = engine.nodes().find(|node| node.mac == peer(11)).expect("here");
    let held = back.desired.or(back.confirmed).expect("in the plan");
...
fn moving_the_bluetooth_scan_never_leaves_two_nodes_scanning_it() {
    ...
    // so the node giving it up can land on the slot the deal has nothing for.
    let clock = Clock::new();
    let config = EngineConfig { pool: ChannelPool::Us, ..EngineConfig::default() };
    let mut engine = engine(config, &clock);
    for n in 0..13 {
    ...
    engine.handle(Event::Command(Command::AssignBle { mac: Some(peer(0)) }), clock.at(20));
    settle_c6_fleet(&mut engine, &clock, 13, 20);

    let scanning: Vec<Mac> = engine
        .nodes()
        .filter(|node| node.desired.or(node.confirmed).is_some_and(|held| held.ble))
        .map(|node| node.mac)
        .collect();
...
fn heartbeats_replayed_out_of_the_bridges_backlog_do_not_open_admin_windows() {
    ...
    for beat in 0..9u32 {
        ...
        // The planner cuts a share the moment the node joins, so every one of
        // these windows is one the engine had something to say in and declined to.
        if beat == 0 {
    ...
    assert_eq!(
        ...
        9,
        "all nine: the first heartbeat is what admits the node and cuts its share, so every \
         window after it owed something"
    );
